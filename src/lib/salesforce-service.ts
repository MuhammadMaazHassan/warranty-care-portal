...
export class SalesforceClient {
  ...
  async query<T = Record<string, unknown>>(soql: string): Promise<SalesforceQueryResult<T>> {
    return this.apiRequest<SalesforceQueryResult<T>>(
   ...
   */
  async queryMore<T = Record<string, unknown>>(nextRecordsUrl: string): Promise<SalesforceQueryResult<T>> {
    return this.apiRequest<SalesforceQueryResult<T>>(nextRecordsUrl);
  }

  /**
   * Create a Bulk API 2.0 query job.
   */
  async createBulkQueryJob(soql: string): Promise<BulkJobInfo> {
    return this.apiRequest<BulkJobInfo>("/services/data/v59.0/jobs/query", {
      method: "POST",
      body: JSON.stringify({
        operation: "query",
        query: soql,
  ...
  async getBulkJobStatus(jobId: string): Promise<BulkJobInfo> {
    return this.apiRequest<BulkJobInfo>(`/services/data/v59.0/jobs/query/${jobId}`);
  }

  /**
   * Get results of a completed Bulk API 2.0 query job as CSV text.
   */
  async getBulkQueryResults(jobId: string): Promise<string> {
   ...
   */
  async updateRecord(sObjectType: string, recordId: string, data: Record<string, unknown>): Promise<void> {
   ...
   */
  async testConnection(): Promise<{ ok: boolean; message: string; identity?: string }> {
    try {
      const result = await this.apiRequest<{ display_name: string; email: string }>(
...
export async function getAuthenticatedClient(companyId: string): Promise<{
  ...
  if (now.getTime() > tokenExpiry.getTime() - bufferMs) {
    ...
    try {
      const refreshResult = await SalesforceClient.refreshAccessToken({
        ...
        clientSecret,
        environment: connection.environment as "production" | "sandbox",
      });
...
export function mapSalesforceRecordToLead(
  sfRecord: Record<string, any>,
  mappings: { salesforceField: string; portalField: string; isConsentField: boolean }[]
): Record<string, any> {
...
export async function seedDefaultMappings(companyId: string): Promise<void> {
