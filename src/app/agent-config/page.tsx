...
export default function AgentConfigPage() {
  ...
  useEffect(() => {
    const loadData = () => {
      const storedVersions = localStorage.getItem("agentConfigVersions");
      if (storedVersions) {
        const parsed: ConfigVersion[] = JSON.parse(storedVersions);
        setVersions(parsed);
        const active = parsed.find((v) => v.isActive);
        if (active) {
  ...
  const handleSave = async (type: "prompt" | "greeting" | "escalation") => {
    ...
    const newVersion: ConfigVersion = {
      ...
      prompt:
        type === "prompt"
          ? prompt
          : versions.find((v) => v.isActive)?.prompt || prompt,
      greeting:
        type === "greeting"
          ? greeting
          : versions.find((v) => v.isActive)?.greeting || greeting,
      escalation:
        type === "escalation"
          ? escalation
          : versions.find((v) => v.isActive)?.escalation || escalation,
      createdAt: new Date().toISOString(),
      isActive: true,
    };
    ...
    showToast(
      "success",
      `${type.charAt(0).toUpperCase() + type.slice(1)} saved as ${newVersion.version}`,
    );
  };
