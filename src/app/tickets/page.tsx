...
const mockCurrentUser = {
  role: "admin", // Change to "homeowner" to test role-based view
  name: "Sarah Johnson",
  homeownerId: "TKT-001", // For demo, matches homeowner name
};

// Full ticket data
const allTicketsData: Ticket[] = [
  {
    id: "TKT-001",
...
export default function TicketsPage() {
  ...
  const isHomeowner = mockCurrentUser.role === "homeowner";
  const currentHomeownerName = mockCurrentUser.name;

  // Load tickets with simulated delay
  useEffect(() => {
    setLoading(true);
    const timer = setTimeout(() => {
      ...
      if (isHomeowner) {
        filteredTickets = filteredTickets.filter(
          (t) => t.homeowner === currentHomeownerName
        );
      }
  ...
  }, [isHomeowner, currentHomeownerName]);
  ...
  const handleRefresh = async () => {
    ...
    let refreshed = [...allTicketsData];
    if (isHomeowner) {
      refreshed = refreshed.filter((t) => t.homeowner === currentHomeownerName);
    }
  ...
  const handleStatusChange = async (ticketId: string, newStatus: TicketStatus) => {
    ...
    showToast("success", `Ticket ${ticketId} status updated to ${newStatus.replace("_", " ")}`);
                        ...
                        >
                          <div className="flex justify-between items-start">
                            <div>
                              ...
                              <div className="text-sm font-semibold mt-1">{ticket.homeowner}</div>
                            </div>
